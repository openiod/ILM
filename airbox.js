...
//		executeSqlStream(query, callback);
	...
	getCbsBuurtProjectEHVAirport: function (param, callback) {
		...
		if (param.objectId == 'geoLocationArea') {
			query = 'select gm_code, gm_naam, bu_code, bu_naam, ST_AsGeoJSON(ST_Simplify(geom4326,0.001)) geojson from get_cbs_buurt_from_point(' + param.lng + ',' + param.lat + ');';
		} else {
	...
	",'BU04390402','BU03630668'";  //gors-noord en nieuwendam voor testen 
			...
			query = 'select bu_code, gm_code, gm_naam, bu_naam, ST_AsGeoJSON(ST_Simplify(geom4326,0.001)) geojson from cbsbuurt2012 where bu_code in (' + param.neighborhood + ');';
		}
